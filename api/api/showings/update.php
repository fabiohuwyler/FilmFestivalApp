...
if (
    ...
    echo json_encode(array("message" => "Unable to update showing. Required data is incomplete."));
    exit();
}
...
// Debug log
error_log('Setting showing properties: ' . print_r([
    'id' => $showing->id,
...
try {
    if($showing->update()) {
        ...
        echo json_encode(array("message" => "Showing was updated."));
    } else {
        http_response_code(503);
        echo json_encode(array("message" => "Unable to update showing."));
    }
} catch(Exception $e) {
    http_response_code(503);
    echo json_encode(array(
        "message" => "Unable to update showing.",
        "error" => $e->getMessage()
    ));
}
?>
